...
public partial class LogParser : ILogParser
{
    ...
    public readonly HashSet<int> IdsAll = CanPackageFactory.RegisteredIds;
    ...
    public IEnumerable<ICanPackageParsed> GetPackages(HashSet<int> filterIds, DateTime start)
    {
        foreach (var scanner in _scanners.Values)
        {
            ...
            foreach (var (_, package) in scanner.ExtractAllPackages(filterIds, startBuffer))
            {
    ...
    public IEnumerable<ICanPackageParsed> GetPackages(HashSet<int> filterIds, DateTime start, DateTime end)
    {
        foreach (var scanner in _scanners.Values)
        {
            var startBuffer = _indexParser.FindNearestBufferByDateTime(start);
