...
public class IdModeCivl : BasePackageParsed
{
    ...
    private static readonly Dictionary<ulong, MessageDefinition> BytesDefinitions = new()
    {
        { 0, new ("ОЖИДАНИЕ") },
        { 1, new ("РАБОТА") },
        { 2, new ("Короткий Внутренний Тест") },
        { 3, new ("Полный Внутренний Тест") },
        { 4, new ("калибровка клапана выдоха") },
        { 5, new ("калибровка датчика кислорода на вдохе") },
        { 6, new ("калибровка FASTOXY в капнографе бокового отведения") },
        { 7, new ("калибровка датчиков давления") },
        { 8, new ("проверка ЭМКВ") },
        { 9, new ("калибровка датчика потока кислорода в СГ") },
        { 10, new ("калибровка генератора потока") },
        { 11, new ("генерация потока") },
        { 12, new ("проверка утечки") },
        { 13, new ("технологический детский режим") },
        { 14, new ("калибровка датчика потока воздуха в СГ") },
        { 15, new ("калибровка преобразователя поток-давление") },
        ...
        { 21, new ("замена датчика кислорода на вдохе") },
        { 22, new ("запуск загрузчика для обновления прошивки КИВЛ") },
        { 23, new ("Инициализация") },
        { 24, new ("калибровка нулей датчиков давления в зависимости от температуры") },
        { 25, new ("тест контура для режима nCPAP") },
    };
